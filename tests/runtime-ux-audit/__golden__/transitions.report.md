...
Mode: static
Total violations: 9 rules, 13 instances (critical: 1, serious: 4, moderate: 4, minor: 0)
Pass: no (1 critical, 4 serious)

| Area | Critical | Serious | Moderate | Minor | Highlights |
|---|---|---|---|---|---|
| Transitions | 1 | 4 | 4 | 0 | bfcache blockers 3, view-transition gaps 7, speculation rules 2, prerender analytics 1 |
...
- Instances (2):
  - `.card` at css/site.css:12:9: `.card { view-transition-name: product-card }`
    view-transition-name "product-card" is on 12 elements; names must be unique, or the browser skips the transition.
  - `.hero` at css/site.css:13:9: `.hero { view-transition-name: hero }`
    view-transition-name "hero" is on 2 elements; names must be unique, or the browser skips the transition.
...
- Rules: 11 total; 9 failed, 0 incomplete, 0 passed, 1 not applicable, 1 skipped (dynamic only), 0 ignored, 0 errors
