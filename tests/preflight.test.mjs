...
import { checkBinary, checkNodeModule, PreflightError, runPreflight } from '../plugins/atelier/lib/preflight.mjs';

// Smoke tests for the public preflight API. The detailed cases live in
// tests/lib/ (find-on-path, check-binary, errors-and-ffmpeg, launch-chromium).
describe('preflight', () => {
  ...
  it('runPreflight rejects with a PreflightError when a check fails', async () => {
    const checks = [
    ...
    await expect(runPreflight(checks)).rejects.toBeInstanceOf(PreflightError);
  });
});
