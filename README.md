...
<img alt="A 32 second demo in a Claude Code terminal: install atelier, run the doctor, ask for social cards, change the brand background and regenerate, audit a page for runtime UX problems and fix them, and watch CI go green." src="demos/overview.webp" width="100%">

<sub>A reconstructed Claude Code session: the install lines, doctor output and audit findings are real command output, and every card and icon was generated by atelier for a demo brand. [Watch it with sound (MP4)](demos/atelier-1.0-trailer.mp4). Built frame by frame from [`demos/trailer`](demos/trailer/index.html) with `npm run demo:trailer`.</sub>
